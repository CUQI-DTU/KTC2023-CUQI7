...
def main():
    ...
    for objectno in range (0,len(mat_files)):
        ...
        step_size_vec = [0.02,0.02,0.04,0.04,0.06,0.06,0.08]
        beta_vec = [5*1e-6, 5*1e-6, 5*1e-6, 5*1e-6, 5*1e-6, 5*1e-6, 5*1e-6]
        ...
        if np.sum(reconstruction_lin_solve==2) >0:
            ...
            # Try with q2 = 3
            while(FLAG5 == 1):
                print("Trying q2 = 3 with initial step_size =",step_size)
                q2 = 3
